...
public class ConversationServiceImpl implements ConversationService {
    ...
    public ConversationResponse startConversation(ConversationCreateRequest request, UserPrincipal principal) {
        ...
        Optional<Long> existingId = conversationMemberRepository
            .findConversationIdBetween(currentUser.getId(), target.getId());
        if (existingId.isPresent()) {
            Conversation conversation = conversationRepository.findById(existingId.get())
    ...
    public void markRead(Long id, UserPrincipal principal) {
        ConversationMember member = ensureMember(id, principal.getId());
    ...
    private ConversationResponse mapConversation(Conversation conversation, Long viewerId) {
        ...
        List<ConversationResponse.ParticipantSummary> participants = members.stream()
            .map(member -> mapParticipant(member.getUser()))
            .toList();

        Message lastMessage = messageRepository.findTopByConversationIdAndDeletedAtIsNullOrderByCreatedAtDesc(conversation.getId())
            .orElse(null);
    ...
    private ConversationResponse.ParticipantSummary mapParticipant(User user) {
